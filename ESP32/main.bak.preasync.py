...
from machine import UART, RTC, WDT
import _thread
...
# Initialize watchdog with 120s timeout
...
def auto_check(now_sec, sunrise_sec, sunset_sec):
    global last_action_time, DOOR_STATE, FAILSAFE
    OPEN_STATE="open"
    CLOSE_STATE="close"
    # account for rolling over one day to the next here
    # now_sec could be a lot lower if the last_action_time were midnight
    if (now_sec - last_action_time < 300) or (now_sec + last_action_time > 86400 + 300):
        return
    #open 10 minutes before sunrise
    ...
    elif sunset_sec + 600 < now_sec < sunset_sec + 600 and DOOR_STATE != CLOSE_STATE:
...
def main():
    ...
    while True:
        try:
            ...
            now = time.localtime()
            #call ntp_sync if we are after when a time change would occur
            if now[3] > 3:
                if LAST_NTP_SYNC_MDAY < now[2]:
                  log(f"[INFO] Attempting scheduled time sync for mday {now[2]}")
                  synced = sync_time()
                  if not synced:
                      log("[WARN] Scheduled time sync failed")
            manage_cache()
            sun_data = load_sun_data()
