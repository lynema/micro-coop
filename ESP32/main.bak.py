...
def send_uart(cmd, retry_count=0, log_response=True):
    ...
    try:
        ...
        if not response:
            ...
            log(f"[UART] cmd: {cmd} reply: {response}")
            return response
...
def auto_check(now_sec, sunrise_sec, sunset_sec):
    ...
    #open at sunrise
    if sunrise_sec < now_sec < sunrise_sec + 600 and DOOR_STATE != OPEN_STATE:
    ...
    if now_sec >= FAILSAFE_OPEN_TO_CLOSED and DOOR_STATE != CLOSE_STATE:
        ...
        send_uart(CLOSE_STATE)
        last_action_time = now_sec
    elif now_sec >= FAILSAFE_CLOSED_TO_OPEN and DOOR_STATE != OPEN_STATE:
